...
import { Layers, ArrowRight, Lock, Mail, ArrowLeft, AlertCircle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#0b0f17] flex flex-col justify-between p-6 text-slate-100 selection:bg-blue-500 selection:text-white">
      ...
      <div className="w-full max-w-md mx-auto glass-panel p-8 rounded-2xl border border-slate-800 shadow-2xl flex flex-col gap-6 my-auto">
        <div className="flex flex-col items-center text-center gap-2">
          ...
          <h1 className="text-xl font-bold text-slate-100">Sign in to DevCanvas</h1>
          <p className="text-xs text-slate-400">Welcome back to your architecture workspace</p>
        </div>
        ...
        <div className="relative flex items-center justify-center">
          ...
          <span className="absolute bg-[#0b0f17] px-3 text-[10px] text-slate-500 uppercase font-bold">Or</span>
        </div>

        <button
          onClick={handleDemoLogin}
          disabled={loading}
          className="w-full py-2.5 rounded-xl glass-card text-xs font-bold text-slate-200 hover:text-white transition-all hover:bg-slate-800"
        >
          Instant Demo Access
        </button>
        ...
        <div className="text-center text-xs text-slate-400">
          ...
          <button onClick={() => onNavigate('register')} className="text-blue-400 font-bold hover:underline">
            Register free
          </button>
        </div>
      </div>
